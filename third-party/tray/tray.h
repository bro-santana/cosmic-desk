...
#include <gtk/gtk.h>
/* COSMIC MODIFICATION: current distributions (e.g. Ubuntu 24.04) ship the
   Ayatana fork, which installs its header under a different name. The original
   libappindicator path is kept as the fallback. */
#if defined(TRAY_AYATANA_APPINDICATOR)
...
static int tray_loop(int blocking) {
  MSG msg;
  /* COSMIC MODIFICATION: upstream dispatches an uninitialized MSG when no
     message is pending, which corrupts the non-blocking pump we call every
     frame. Bail out unless a message was actually retrieved. */
  if (blocking) {
