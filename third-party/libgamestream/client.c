...
#include <stdio.h>
#include <string.h>
// COSMIC MODIFICATION: upstream is Linux-only; htonl() lives in winsock2.h on
// Windows and arpa/inet.h on POSIX, and _mkdir() lives in direct.h.
#ifdef _WIN32
#include <winsock2.h>
#include <direct.h>
#else
...
static int mkdirtree(const char* directory) {
  ...
  while (*p != 0) {
    ...
    *p = 0;
    ...
    // COSMIC MODIFICATION: POSIX mkdir() takes a mode argument; Windows
    // _mkdir() does not.
#ifdef _WIN32
...
int gs_start_app(PSERVER_DATA server, STREAM_CONFIGURATION *config, int appId, bool sops, bool localaudio, int gamepad_mask) {
  ...
  char url[4096];
  // COSMIC MODIFICATION: u_int32_t is a BSD-ism; uint32_t is standard C.
  uint32_t rikeyid = 0;
