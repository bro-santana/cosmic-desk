...
| A3 | **Rendering order per frame (Bridge mode):** SDL draws the scene (bg gradient → twinkles → nebula → stars-far → stars-mid → planets → orbit ring → beams → desk group → glow/reflex → vignette → warp flash), then the existing `ImGui_ImplSDLRenderer2_RenderDrawData` draws all UI on top. | Mirrors the prototype's DOM stacking (the HTML puts stars-far/stars-mid above planets); reuses today's main-loop structure (main.cpp:766–841) unchanged in shape. |
