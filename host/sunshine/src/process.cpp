...
namespace proc {
  ...
  void proc_t::terminate() {
    ...
    if (proc::proc.get_last_run_app_name().length() > 0 && has_run) {
      // COSMIC MODIFICATION: system_tray::update_tray_stopped() removed — Cosmic
      // Desk has its own tray (M0).
  ...
  // COSMIC MODIFICATION: apps.json/multi-app logic replaced with a hardcoded single
  // "Desktop" app (plan M1.3). Empty command = stream the desktop (Sunshine
  // convention). The app id must be numeric because nvhttp.cpp parses "appid" as an
  // int and proc_t::execute() compares it against ctx_t::id via std::to_string().
  std::optional<proc::proc_t> parse(const std::string &file_name) {
    auto this_env = boost::this_process::environment();

    proc::ctx_t desktop_app;
    desktop_app.name = "Desktop";
    desktop_app.cmd = "";
    desktop_app.id = "1";
    desktop_app.elevated = false;
    desktop_app.auto_detach = true;
    desktop_app.wait_all = true;
    desktop_app.exit_timeout = std::chrono::seconds {5};

    std::vector<proc::ctx_t> apps;
    apps.emplace_back(std::move(desktop_app));
