...
// COSMIC MODIFICATION: include the full nanors rs.h (bundled in the
// moonlight-common-c submodule) so consumers such as stream.cpp can access
// reed_solomon members (e.g. `p`). Upstream only forward-declared the struct
// because its standalone nanors copy was compiled into rswrapper.c; the bundled
// nanors is compiled separately, so the definition must be visible here.
#include <rs.h>
