...
// local includes
// COSMIC MODIFICATION: cbs.h deleted — it depended on Sunshine's patched FFmpeg
// libcbs (plan R3/D4); SPS/VPS injection and validation are removed below.
#include "config.h"
...
namespace video {
  ...
  // COSMIC MODIFICATION: the QSV (quicksync) encoder is dropped (plan R3/D4 — the
  // Windows encoder floor is NVENC + x264 only), so its profile enums are removed.
  ...
  public:
    ...
    avcodec_encode_session_t(avcodec_ctx_t &&avcodec_ctx, std::unique_ptr<platf::avcodec_encode_device_t> encode_device):
    ...
    // COSMIC MODIFICATION: cbs-based SPS/VPS injection removed (cbs.h deleted, plan
    // R3/D4). `replacements` stays empty; encoders emit their own SPS/VPS in-band.
  };
  ...
  // COSMIC MODIFICATION: the QSV (quicksync) and MediaFoundation (mediafoundation)
  // encoders are dropped (plan R3/D4 — Windows encoder floor is NVENC + x264 only;
  // AMF was already dropped). The whole `#ifdef _WIN32` block is removed.
  ...
  static const std::vector<encoder_t *> encoders {
    ...
    &nvenc,
#endif
    // COSMIC MODIFICATION: QSV, AMF and MediaFoundation encoders removed from the
    // encoder list (plan R3/D4 — Windows encoder floor is NVENC + x264 only).
#if defined(__linux__) || defined(linux) || defined(__linux) || defined(__FreeBSD__)
  #ifdef SUNSHINE_BUILD_VULKAN
    &vulkan,
  #endif
  ...
  int encode_avcodec(int64_t frame_nr, avcodec_encode_session_t &session, safe::mail_raw_t::queue_t<packet_t> &packets, void *channel_data, std::optional<std::chrono::steady_clock::time_point> frame_timestamp) {
    ...
    // COSMIC MODIFICATION: cbs-based SPS/VPS injection removed (cbs.h deleted,
    // plan R3/D4); encoders emit their own SPS/VPS in-band.
  ...
  std::unique_ptr<avcodec_encode_session_t> make_avcodec_encode_session(
    ...
    auto session = std::make_unique<avcodec_encode_session_t>(
      ...
      // COSMIC MODIFICATION: cbs-based SPS/VPS injection removed (cbs.h deleted,
      // plan R3/D4); the inject parameter was dropped.
    );
  ...
  int validate_config(std::shared_ptr<platf::display_t> disp, const encoder_t &encoder, const config_t &config) {
    ...
    if (config.videoFormat <= 1) {
      if (auto packet_avcodec = dynamic_cast<packet_raw_avcodec *>(packet.get())) {
        // COSMIC MODIFICATION: cbs::validate_sps() removed (cbs.h deleted, plan
        // R3/D4). Assume the encoder emits SPS with VUI parameters, which is the
        // common case for the encoders we build against system FFmpeg.
        (void) packet_avcodec;
        flag |= VUI_PARAMS;
      } else {
