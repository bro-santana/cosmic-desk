...
// lib includes
// COSMIC MODIFICATION: <AMF/core/Factory.h> removed — the AMF encoder path is
// dropped (plan R3/D4) and MSYS2 ships no AMF headers.
...
#undef create_pixel_shader_helper
  ...
  // COSMIC MODIFICATION: display_wgc_vram_t (Windows.Graphics.Capture backend)
  // removed — MSYS2 lacks the winrt headers it needs (plan R3/D4).
  ...
  bool display_vram_t::is_codec_supported(std::string_view name, const ::video::config_t &config) {
    ...
    if (adapter_desc.VendorId == 0x1002) {  // AMD
      // COSMIC MODIFICATION: the AMF encoder path is dropped (plan R3/D4), so no
      // AMD GPU is compatible with any remaining encoder.
      return false;
