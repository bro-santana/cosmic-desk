...
#include <dxgi1_6.h>
#include <Unknwn.h>
// COSMIC MODIFICATION: <winrt/windows.graphics.capture.h> removed — the
// Windows.Graphics.Capture backend was deleted (MSYS2 lacks winrt headers, plan R3/D4).
...
namespace platf::dxgi {
  ...
  // COSMIC MODIFICATION: the Windows.Graphics.Capture (WGC) backend was deleted —
  // MSYS2 lacks the winrt headers it needs (plan R3/D4). DXGI duplication
  // (display_ddup_ram_t / display_ddup_vram_t) is the primary capture path.
}  // namespace platf::dxgi
