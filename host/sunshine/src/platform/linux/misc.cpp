...
// COSMIC MODIFICATION: upstream entry_handler.h was deleted (web UI/service entry
// point dropped, plan D5); this shim provides lifetime::exit_sunshine()/get_argv().
#include "src/entry_handler_shim.h"
  ...
  fs::path appdata() {
    ...
    std::call_once(migration_flag, []() {
      ...
      if ((dir = getenv("CONFIGURATION_DIRECTORY")) != nullptr && strlen(dir) > 0) {
        found = true;
        // COSMIC MODIFICATION: config dir is "cosmicdesk" (plan D6), not "sunshine".
        config_path = fs::path(dir) / "cosmicdesk"sv;
      }
      ...
      if (!found) {
        ...
        // COSMIC MODIFICATION: config dir is "cosmicdesk" (plan D6), not "sunshine".
        config_path = fs::path(homedir) / ".config/cosmicdesk"sv;
      }
