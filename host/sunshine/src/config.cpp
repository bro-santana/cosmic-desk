...
// local includes
#include "config.h"
// COSMIC MODIFICATION: upstream entry_handler.{h,cpp} was deleted (web UI + service
// entry point dropped, plan D5); this shim provides the few symbols we still use.
#include "entry_handler_shim.h"
...
namespace config {
  ...
  namespace amd {
// COSMIC MODIFICATION: the AMF encoder path is dropped (plan R3/D4 — MSYS2 has no
// AMF headers and the amdvce encoder was removed from video.cpp). Always use the
// hardcoded constant values instead of including the AMF SDK headers on Windows.
#if 1
...
#undef _CONVERT_
  ...
  // COSMIC MODIFICATION: the qsv config namespace is removed (QSV encoder dropped,
  // plan R3/D4 — Windows encoder floor is NVENC + x264 only).
...
#undef _CONVERT_2_ARG_
  ...
  video_t video {
    ...
    // COSMIC MODIFICATION: qsv defaults removed (QSV encoder dropped, plan R3/D4).
  ...
  sunshine_t sunshine {
    ...
    {},  // Password
    {},  // Password Salt
    // COSMIC MODIFICATION: default config file is host.conf (plan D6/M1.3),
    // generated by src/hostglue/host.cpp from cosmic.json's port_base.
    platf::appdata().string() + "/host.conf",  // config file
    {},  // cmd args
    ...
    // COSMIC MODIFICATION: `system_tray` removed — Cosmic Desk owns its tray (M0).
    {},  // prep commands
  };
  ...
  void apply_config(std::unordered_map<std::string, std::string> &&vars) {
    ...
    // COSMIC MODIFICATION: qsv_* config keys are no longer parsed (QSV encoder
    // dropped, plan R3/D4); unknown keys are logged, so stale configs stay valid.
    ...
    // COSMIC MODIFICATION: apps.json is unused by Cosmic Desk — process.cpp is
    // stubbed to a single hardcoded "Desktop" app and never reads the apps file.
    // The upstream copy-from-assets block is deleted; without a shipped apps.json
    // it made config::parse() throw and the host refuse to start.
    ...
    // COSMIC MODIFICATION: `system_tray` config key removed — Cosmic Desk owns its tray (M0).
    ...
    // COSMIC MODIFICATION: UPnP dropped (plan D5); the "upnp" config key is left
    // unparsed so the config parser stays tolerant of unknown keys (it logs them).
